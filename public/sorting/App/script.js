let bars = [];
const def = "#034078" //pink 
const chng = "#1282a2" 
const finished = "#8ef511" //green
const selected = "rgb(10, 10, 77)";
...
//This function dynamically generates a set of bars with random heights and displays them 
/*
function generateBars(n = -1) {
	...
	for (let i = 0; i < n; i++) {
		const height = Math.floor(2 + Math.random() * 98);
...
function generateBars(n = -1) {
	...
	for (let i = 0; i < n; i++) {
		const height = Math.floor(2 + Math.random() * 98);
		bars.push(
			'<div class="bar-container">' + // Container for div and span
			'<div class="bar" id="' + i + '" style="height:' + height + '%"></div>' +
			'<span class="bar-label">' + height + '</span>' +
			'</div>' // Closing bar-container div
		);
	}
	container.innerHTML = bars.join('');
}
